...
parameters <- paste0("t=", movie.no.spaces, "&y=&plot=short&r=json")
...
movie.list <- c("The Dark Knight Rises", "Frozen", "Good Will Hunting")
# Remove spaces from each movie title
movie.list.remove.spaces <- gsub(" ", "+", movie.list)
# Prepare this list to be passed in as parameters for the API
movie.list.parameters <- paste0("t=", movie.list.remove.spaces, "&y=&plot=short&r=json")
# Create API URL request and assign it to the variable api.request
api.request <- paste0(base.url, movie.list.parameters)
